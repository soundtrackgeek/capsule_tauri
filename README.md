...
- Backup-guarded star, unstar, pin, unpin, hide, unhide, and confirmed delete
  entry actions.
- Full-page markdown composer with metadata fields, mood/tag autocomplete,
  save-time entry dating, continuation UUID support, multi-select queued local
  image attachments with thumbnail previews, writing stats, and local draft
  recovery.
