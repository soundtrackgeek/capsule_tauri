...
pub fn search_entries_for_database(db_path: &Path, input: SearchRequest) -> Result<SearchResponse> {
    ...
    // A CLI query must remain a true read.  The desktop adapter retains the
    // repair-capable search wrapper for legacy behavior, while this headless
    // path deliberately uses the repair-free projection.
    let response = entries::list_entries_read_only_for_database(db_path, parsed.filters)?;
    Ok(SearchResponse {
...
fn search_with_fts(
    ...
    let entries = entries::list_entries_by_uuids_read_only_for_database(db_path, &uuids)?;
    Ok(SearchResponse {
        entries,
        total,
        limit,
